...
const DEFAULT_BLOG_POSTS = [
  {
    ...
    tag: "お知らせ",
    url: "/post/blog-launched-20260424",
    image: "/src/assets/thumbnail/noimg.png",
  },
];

function getCardHtml(post, index, basePath) {
  const imagePath = post.image || "/src/assets/thumbnail/noimg.png";
...
// Initialize BLOG_POSTS from embedded JSON if present
(function initEmbeddedPosts() {
  if (!Array.isArray(window.BLOG_POSTS)) {
    const el = document.getElementById('blog-posts');
    if (el) {
      try {
        window.BLOG_POSTS = JSON.parse(el.textContent || el.innerText || '[]');
      } catch (e) {
        console.warn('Failed to parse embedded blog posts JSON', e);
        window.BLOG_POSTS = [...DEFAULT_BLOG_POSTS];
      }
    } else {
      window.BLOG_POSTS = [...DEFAULT_BLOG_POSTS];
    }
